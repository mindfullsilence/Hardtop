<?php
 ...
 #--------------------------------------------------------[ Photo specific ]---
  ...
  private function thumbphoto() {
    if ($this->page["Title"] == "") $this->openpage ("Title");
    //preg_match("!id\=\"title-overview-widget-layout\">[^<]*<a[^<]+<img src\=\"(.+?)\"/i",$this->page["Title"],$match);
    preg_match('/id="title-overview-widget-layout">.*?<img src="(.+?)"/is',$this->page["Title"],$match);
    if (empty($match[1])) return FALSE;
    $this->main_thumb = $match[1];
    preg_match('|(.*\._V1).*|iUs',$match[1],$mo);
    if (empty($mo[1])) return FALSE;
   ...
   * @see IMDB page / (TitlePage)
   */
  public function photo($thumb=false) {
   ...
   * @see IMDB page / (TitlePage)
   */
  public function savephoto($path,$thumb=false,$rerun=0) {
    switch ($rerun) {
